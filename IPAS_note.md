...
>* -加強保護集中存放通行碼的伺服器
# http port> 80
# ICMP port> 我們在網絡中經常會使用到ICMP協議，只不過我們覺察不到而已。比如我們經常使用的用於檢查網絡通不通的Ping命令，這個「Ping」的過程實際上就是ICMP協議工作的過程。還有其他的網絡命令如跟蹤路由的Tracert命令也是基於ICMP協議的。所以ICMP沒有埠,他不是TCP or UDP 其中的一份子,他和TCP,UDP是對等的存在
